...
**Fase corrente: validazione pre-codice.** Prima di scrivere una riga di Go, il vincolo va
testato catturando manualmente le decisioni di un repository reale — questo stesso repo — per
un periodo di prova, osservando onestamente dove si rompe l'abitudine. Non scrivere codice per
"automatizzare" questa fase finché la validazione manuale non ha dato un segnale chiaro.
